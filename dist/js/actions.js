...
function replaceRequest() {
  let sendingData = new FormData();
  sendingData.append('id_event', document.querySelector("#replace-event-event").value);
  ...
  getJSON("/api/requests/replace", sendingData, data => {
    ...
    supervisorsMulti.forEach(s => $(`${s.class} option[value="${replaceEventID}"]`).remove());
...
function getComments() {
  getJSON("/api/comments/getAll", null, data => {
  	data.forEach(d =>{
  		renderComment(d);
  	})
  })
...
function sendComment() {
  let val = document.querySelector(".send-comment").value;
  if ( document.querySelector(".send-comment").value.length < 5) return;
  let sendingData = new FormData();
  sendingData.append('text', val);
  getJSON("/api/comments/insert", sendingData, data => {
...
function hideOpenEditorMember(id, el) {
  ...
  if (a.style.display == "none") {
    ...
    let memberBlock = document.querySelector(`[data-member-block="${id}"]`).children;
    let sendingData = new FormData();
    sendingData.append('id', id);
    sendingData.append('typeInst', memberBlock[0].children[0].children[0].value);
    sendingData.append('lname', memberBlock[1].children[0].children[0].value);
    sendingData.append('num_stud', memberBlock[2].children[0].children[0].value);
    sendingData.append('stip', document.querySelector(`[name="` + memberBlock[3].children[0].children[0].name + `"]:checked`).value);
    ...
    sendingData.append('name_org', memberBlock[7].children[0].children[0].value);
    sendingData.append('phone', memberBlock[8].children[0].children[0].value);
    getJSON("/api/members/update", sendingData, data => {
      let memberDesc = document.querySelector(`[data-member-desc="${id}"]`).children[0].children[0].children;
      memberDesc[0].innerText = `${data[3]} ${data[4]} ${data[5]}`
      if (data[0] == 0) {
        memberDesc[0].innerText = `${data[3]} ${data[4]} ${data[5]}`;
        memberDesc[1].innerText = `№${data[6]} ${data[7] == 1 ? "Бюджет" : "Контракт"} гр. ${data[13]}`
      }
      if (data[0] == 1) {
      ...
      if (data[1] == 2) {
        memberDesc[1].innerText = memberBlock[6].children[0].children[0].value + " " + memberBlock[7].children[0].children[0].value
      }
...
function appendEvent() {
  ...
  sendingData.append('conf-id', document.querySelector("#add-event-conf-id").value);
  ...
  sendingData.append('location', document.querySelector("#add-event-location").value);
...
function requestAction(action, id = 0, el = null) {
  ...
  if (action == 'showUsers') {
    searchUserManual(document.querySelector(".searching").value, document.querySelector("#showUsers:checked").value)
  }
  if (action == "append_r") {
    let sendingData = new FormData();
    ...
    getJSON("/api/requests/append", sendingData, data => {
      ...
      supervisorsMulti.forEach(s => {
        let o = new Option('Безымянный (ID ' + data[0].ID + ')', data[0].ID, !1, !1);
        $(s.class).append(o).trigger('change')
      });
      recsList.forEach(r => {
        let o = new Option('Безымянный (ID ' + data[0].ID + ')', data[0].ID, !1, !1);
        $(`[data-recommendation="${r}"]`).append(o).trigger('change')
      })
    })
  }
  if (action == "user_role") {
  ...
  if (action == "remove_juri") {
    let sendingData = new FormData();
    sendingData.append('id', id);
    getJSON("/api/juri/remove", sendingData, data => {
      document.querySelector(`[data-juriblock="${id}"]`).remove();
    })
  }
  ...
  if (action == "r_name") {
    ...
    getJSON("/api/requests/update", sendingData, data => {
      ...
      supervisorsMulti.forEach(s => {
        let tmp = !1;
        $(s.class).select2('val').forEach(s2 => {
          if (s2 == id) tmp = !0
        })
      ...
      recsList.forEach(r => {
        let tmp = !1;
        $(`[data-recommendation="${r}"]`).select2('val').forEach(s2 => {
          if (s2 == id) tmp = !0
        })
  ...
  if (action == "remove_request") {
    if (!confirm(`Вы действительно хотите удалить ${textTpEvent[typeEvent][3]}?`)) {
      return
    }
    let sendingData = new FormData();
    sendingData.append('id', id);
    getJSON("/api/requests/remove", sendingData, data => {
      ...
      if (tmp > 0) requestsList.splice(tmp, 1);
      supervisorsMulti.forEach(s => $(`${s.class} option[value="${id}"]`).remove());
      recsList.forEach(r => {
        $(`[data-recommendation="${r}"] option[value="${id}"]`).remove()
      });
  ...
  if (action == "moderator") {
    ...
    getJSON("/api/requests/moderator", sendingData, data => {
      document.querySelector(`[data-r-block="${id}"]`).className = "card card-primary card-outline"
      el.remove()
    })
  }
